...
import ir.saltech.gradle.types.LicenseTemplate
import ir.saltech.gradle.util.submit
import java.io.File
...
abstract class ValidateLicenseTask @Inject constructor(private val workerExecutor: WorkerExecutor) :
  ...
  fun add() {
    ...
    for (currentFile in files.get()) {
      queue.submit<ir.saltech.gradle.tasks.ValidateLicenseWorkAction, ir.saltech.gradle.tasks.ValidateLicenseWorkParameters> {
 ...
 */
abstract class ValidateLicenseWorkAction : WorkAction<ir.saltech.gradle.tasks.ValidateLicenseWorkParameters> {
